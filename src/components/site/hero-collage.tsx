...
export function HeroCollage({ className }: { className?: string }) {
 ...
 <p
 className={cn(
 "shrink-0 truncate border-b px-2.5 py-1 font-mono text-[10px] leading-4 ",
 !reduceMotion && active === i
 ? "border-border/70"
 : "border-border/40"
 )}
 ...
 href={t.href}
 className={cn(
 "block truncate outline-none hover:text-foreground focus-visible:underline",
 !reduceMotion && active === i
 ? "text-foreground"
 ...
 className="absolute right-3 bottom-3 z-10 flex size-7 items-center justify-center rounded-full border border-border bg-background/80 text-muted-foreground backdrop-blur-sm hover:text-foreground focus-visible:outline-2 focus-visible:outline-offset-2"
 >
